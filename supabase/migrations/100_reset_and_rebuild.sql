-- 100_reset_and_rebuild.sql
-- Single authoritative schema replacing all prior migrations (009-016).
-- Run ONLY after DROP SCHEMA public CASCADE; CREATE SCHEMA public; + auth.users cleared.
-- No caption_fts column. No retrieve_candidates_text RPC.
-- Reference retrieval is an inline PostgREST query in generate_orchestrated.
...
-- Reference images live at: reference-images/studio/{user_id}/{style}/{room}/{uuid}.{ext}
-- ---------------------------------------------------------------------------

-- Ensure bucket exists (idempotent; safe to run if already created in dashboard)
...
-- reference-images: authenticated users can read their own objects
...
-- user-outputs: scoped to outputs/{user_id}/...
...
-- ---------------------------------------------------------------------------
-- App config seed (model IDs — update values without schema changes)
-- ---------------------------------------------------------------------------
